...
fn find_package(name: &str) -> Vec<PathBuf> {
    ...
    println!("Checking config for linux-pkg-config feature: {}", cfg!(all(target_os = "linux", feature = "linux-pkg-config")));
    let no_pkg_config_var_value = std::env::var(no_pkg_config_var_name);
    println!("Checking NO_PKG_CONFIG variable value: {:?}", no_pkg_config_var_value);
    
    if cfg!(all(target_os = "linux", feature = "linux-pkg-config"))
        && no_pkg_config_var_value.as_deref() != Ok("1")
    {
        println!("Using pkg-config");
...
fn generate_bindings(
    ...
    println!("Generating bindings for: {}", ffi_header.display());
...
fn gen_vcpkg_package(package: &str, ffi_header: &str, generated: &str, regex: &str) {
    println!("Generating vcpkg package for: {}", package);

    let includes = find_package(package);
    println!("Includes found: {:?}", includes);
    ...
    println!("FFI header path: {}", ffi_header.display());
    println!("rerun-if-changed={}", ffi_header.display());

    for dir in &includes {
        println!("Include directory: {}", dir.display());
        println!("rerun-if-changed={}", dir.display());
    }

    let ffi_rs = out_dir.join(generated);
    println!("Generated FFI Rust source path: {}", ffi_rs.display());

    let exact_file = src_dir.join("generated").join(generated);
    println!("Exact file path: {}", exact_file.display());

    generate_bindings(&ffi_header, &includes, &ffi_rs, &exact_file, regex);
    println!("Bindings generated for package: {}", package);
}
...
fn main() {
    ...
    println!("Target info retrieved.");

    if let Ok(target) = target {
        println!("Target pointer width: {}", target.target_pointer_width());
        if target.target_pointer_width() != "64" {
            println!("Only support 64bit system");
            // panic!("Only support 64bit system");
        }
    } else {
        println!("Failed to retrieve target info.");
    }

    env::remove_var("CARGO_CFG_TARGET_FEATURE");
    println!("Removed CARGO_CFG_TARGET_FEATURE environment variable.");
    env::set_var("CARGO_CFG_TARGET_FEATURE", "crt-static");
    println!("Set CARGO_CFG_TARGET_FEATURE to crt-static.");

    println!("Finding package: libyuv");
    find_package("libyuv");
    println!("Generating vcpkg package: libvpx");
    gen_vcpkg_package("libvpx", "vpx_ffi.h", "vpx_ffi.rs", "^[vV].*");
    println!("Generating vcpkg package: aom");
    gen_vcpkg_package("aom", "aom_ffi.h", "aom_ffi.rs", "^(aom|AOM|OBU|AV1).*");
    println!("Generating vcpkg package: libyuv");
    gen_vcpkg_package("libyuv", "yuv_ffi.h", "yuv_ffi.rs", ".*");
    // println!("Generating ffmpeg package");
    // ffmpeg();
    ...
    if target_os == "ios" {
        // nothing
        println!("Target OS is iOS, doing nothing.");
    } else if target_os == "android" {
        println!("Target OS is Android, setting cargo cfg to android.");
        println!("cargo:rustc-cfg=android");
    } else if cfg!(windows) {
        // The first choice is Windows because DXGI is amazing.
        println!("Target OS is Windows, setting cargo cfg to dxgi.");
        println!("cargo:rustc-cfg=dxgi");
    } else if cfg!(target_os = "macos") {
        // Quartz is second because macOS is the (annoying) exception.
        println!("Target OS is macOS, setting cargo cfg to quartz.");
        println!("cargo:rustc-cfg=quartz");
    } else if cfg!(unix) {
        // On UNIX we pray that X11 (with XCB) is available.
        println!("Target OS is Unix, setting cargo cfg to x11.");
